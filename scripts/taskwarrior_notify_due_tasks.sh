...
export TASK_BIN
export TW_TASK_FILTER
export TW_COMMAND_TIMEOUT_SECONDS
export TW_INCLUDE_PROJECTS TW_EXCLUDE_PROJECTS TW_INCLUDE_TAGS TW_EXCLUDE_TAGS TW_OPTOUT_TAG
...
if ! python3 -m taskwarrior_tnt.notifier \
    "$WINDOW_PAST_HOURS" "$WINDOW_FUTURE_HOURS" "$MAX_TASKS" \
    "$SNOOZE_FILE" "$GUI_CACHE_FILE" "$NOTIFICATION_CONFIG_SIGNATURE" \
    > "$records_file"; then
